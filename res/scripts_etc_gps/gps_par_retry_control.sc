...
if (-e $PRESENTDATE) then
   ...
   if (-e flag_file_par) then
      ...
      #
      # 4 uur geleden (dit script wordt b.v. te 10.50 aangeroepen om de 06.00 UTC file op te halen)
      #

      #huidige tijd 4 uur terug (format JJJJMMDDUU, dus minuten verwaarloosd)
      set dtgw4 = `$BACKUPDATE $dtg -4`
      ...
      cp $ENV_UBA_GLOBAL/LAMH_UBA_$dtgw4"00_00000_TW" $ENV_GPS_LOCAL/input_hirlam  
      ...
      cp $ENV_TMA_GLOBAL/NEWH_TMA_$dtgw4"00_XX000_TW" $ENV_GPS_LOCAL/input_nedwam  
      ...
      #
      cd $ENV_GPS_LOCAL/input_nedwam
      if (-z NEWH_TMA_$dtgw4"00_XX000_TW" || !(-e NEWH_TMA_$dtgw4"00_XX000_TW")) then
         ...
         cd $ENV_GPS_LOG
         echo `date` >>& log.txt
         echo NEWH_TMA_$dtgw4"00_XX000_TW and/or hier bij corresponderende LAMH_UBA_xxxxxxxxxxxx_00000_TW empty or not present" >>& log.txt
         echo "************************************************************************************" >>& log.txt
 
      else

         cd $ENV_GPS_LOCAL/input_hirlam
         if (-z LAMH_UBA_$dtgw4"00_00000_TW" || !(-e LAMH_UBA_$dtgw4"00_00000_TW")) then
            ...
            cd $ENV_GPS_LOG
            echo `date` >>& log.txt
            echo LAMH_UBA_$dtgw4"00_00000_TW empty or not present" >>& log.txt
            echo "************************************************************************************" >>& log.txt
 
         else

            # files (nedwam-TMA en hirlam-UBA) zijn dus nu WEL aanwezig
            ...
            cd $ENV_GPS_LOCAL/input_nedwam
            cp NEWH_TMA_$dtgw4"00_XX000_TW" TSF_IN

            cd $ENV_GPS_LOCAL/input_hirlam
            cp LAMH_UBA_$dtgw4"00_00000_TW" TSF_IN
