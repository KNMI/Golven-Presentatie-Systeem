...
if (-e $PRESENTDATE) then
   ...
   if (-e flag_file_par) then
      ...
      #
      # 8 uur geleden (dit script wordt b.v. te 14.55 aangeroepen om op de 06.00 UTC run file te werken)
      #

      #huidige tijd 8 uur terug (format JJJJMMDDUU, dus minuten verwaarloosd)
      set dtgw8 = `$BACKUPDATE $dtg -8`
      ...
      if (!(-z NEWH_TMA_$dtgw8"00_XX000_TW") && -e NEWH_TMA_$dtgw8"00_XX000_TW") then

         cd $ENV_GPS_LOCAL/input_hirlam
         if (!(-z LAMH_UBA_$dtgw8"00_00000_TW") && -e LAMH_UBA_$dtgw8"00_00000_TW") then
            ...
            # 
            cd $ENV_GPS_LOCAL/input_nedwam
            cp NEWH_TMA_$dtgw8"00_XX000_TW" TSF_IN

            cd $ENV_GPS_LOCAL/input_hirlam
            cp LAMH_UBA_$dtgw8"00_00000_TW" TSF_IN
            ...
            #
            # nedwam parameters -derde pagina- (hs10, dir hs10, hs7)
            # 
