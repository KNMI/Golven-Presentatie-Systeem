...
main(int c, char *argv[])
{
   ...
   /* outputfile naam opbouwen b.v. 2000010106 */
   sprintf(outputfilenaam, "%s", "input_cic/"); // sub directory
   strcat(outputfilenaam, argv[1]);             // b.v. input_cic/2006010106
   strcat(outputfilenaam, ".HWA");              // file name extension   

   extractiefile = fopen(outputfilenaam, "w");  // output file 

   /* kolom aanduidingen regel */
   fprintf(extractiefile, "___id ___la ____lo _dd ff _Hwa Pw YYYYMMDDHH");
   ...
   for (i = 0; i < 2; i++)
   {
      ...
      while ((synopptr = rtwndb_access_getobs(&rtac)) != NULL)
      {
         ...
         {         
            ...
            /* ff (altijd in 0.1 m/s in de RTWNDB)     */
            ...
            {  
               if ( ((synopptr-> s1_ff + 5) / 10) < 100)
                  fprintf(extractiefile, "%02d", (synopptr-> s1_ff + 5) / 10);
               else
                  fprintf(extractiefile, "//");
            } 
            else
            ...
            /* iw (staat zoals ingezonden; echter in de RTWNDB  */
            /* is de ff altijd in m/s)                          */


         /* HwaHwaHwa (gemeten golven nauwkeurig) */
//       if (TLBX_PRESENT("s2_Hwa", &rtac))
//         fprintf(extractiefile, "%03d", synopptr-> s2_Hwa);
//       else // geen Pw beschikbaar
//       {
//         fprintf(extractiefile, "///");
//       } // else

            /* Hwa (gemeten golfhoogte in 0.1 meter eenheden) */
            if (TLBX_I_CONVERT("s2_Hwa", "MTR", 1, &rtac, &valint) == 1)
            {
               /*fprintf(extractiefile, "%03d", valint);*/
               golfhoogte = (float)valint / 10;
               fprintf(extractiefile, "%4.1f", golfhoogte);
            }
//...........
            else if ( (searchtype == SYNOP_STATION) && (TLBX_I_CONVERT("Hwa", "MTR", 1, &rtac, &valint) == 1) )
            {
               // DIT GEEFT OP HALVE METER AFGERONDE WAARDEN !!!!
               /*fprintf(extractiefile, "%03d", valint);*/
               golfhoogte = (float)valint / 10;
               fprintf(extractiefile, "%4.1f", golfhoogte);
            }
...
//............
            ...
            /* Pwa (gemeten golfperiode) */
            if (TLBX_PRESENT("Pwa", &rtac))
               fprintf(extractiefile, "%2d", synopptr-> Pwa);
            else
               fprintf(extractiefile, "//");
         ...
         /* Hwa */
//         /* n.b. HLFMTR werkt niet ! het moet in meter in tienden */
//         if (TLBX_I_CONVERT("Hwa", "MTR", 1, &rtac, &valint) == 1)
//         {
//             /*fprintf(extractiefile, "%03d", valint);*/
//             golfhoogte = (float)valint / 10;
//             fprintf(extractiefile, "%4.1f", golfhoogte);
